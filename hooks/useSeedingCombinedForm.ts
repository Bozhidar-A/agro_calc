...
import {
  CreateDefaultValues,
  CreateZodSchemaForPlantRow,
  UpdateSeedingComboAndPriceDA,
  ValidateMixBalance,
  ...
  function UpdateFinalData(data: CombinedFormValues) {
    ...
    const combinedData: CombinedCalcDBData = {
      ...
      totalPrice: parseFloat(
        RoundToSecondStr(
          data.legume.reduce((acc: number, curr) => acc + curr.priceSeedsPerAcreBGN, 0) +
    ...
    const subscription = form.watch((_value, { name }) => {
      ...
      if (name.includes('dropdownPlant') && rowData.dropdownPlant) {
        ...
        if (selectedPlant) {
          ...
          type FormPath =
            | keyof CombinedFormValues
            | `${keyof CombinedFormValues}.${number}.id`
            | `${keyof CombinedFormValues}.${number}.plantType`;
      ...
      if (name.includes('active')) {
        type FormFieldPath =
          `${keyof CombinedFormValues}.${number}.${'seedingRate' | 'participation'}`;
        ...
        if (!rowData.active) {
          ...
          if (selectedPlant && rowData.seedingRate) {
            if (
              rowData.seedingRate < selectedPlant.minSeedingRate ||
              rowData.seedingRate > selectedPlant.maxSeedingRate
            ) {
      ...
      if (
        ...
        name.includes('seedingRate') ||
        name.includes('dropdownPlant')
      ) {
