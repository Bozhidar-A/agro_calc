...
import {
  AcresToHectares,
  CalculateChemProtRoughSprayerCount,
  CalculateChemProtTotalChemicalLiters,
  CalculateChemProtTotalWorkingSolutionLiters,
  CalculateChemProtWorkingSolutionPerSprayerML,
  HectaresToAcres,
} from '@/lib/math-util';
...
export default function useChemProtWorkingForm() {
    ...
    const fetchChemicals = async () => {
      try {
        ...
        if (!res.success) {
          Log(
            ['calc', 'chem-protection', 'working-solution', 'chemicals', 'BACKGROUND'],
            `Error fetching chemicals: ${res.message}`
          );
          return;
        }
    ...
    const fetchSowingHistory = async () => {
      try {
        ...
        if (!res.success) {
          Log(
            ['calc', 'sowing', 'history', 'BACKGROUND'],
            `Error fetching sowing history: ${res.message}`
          );
          return;
        }
  ...
  useEffect(() => {
    form.trigger().then(() => {
      ...
      const {
        chemicalPerAcreML,
        workingSolutionPerAcreLiters,
        sprayerVolumePerAcreLiters,
        areaToBeSprayedAcres,
      } = form.getValues();
      let isMathWorking = true;

      const totalChemicalLiters = CalculateChemProtTotalChemicalLiters(
        chemicalPerAcreML,
        areaToBeSprayedAcres
      );
      const totalWorkingSolutionLiters = CalculateChemProtTotalWorkingSolutionLiters(
        workingSolutionPerAcreLiters,
        areaToBeSprayedAcres
      );
      const roughSprayerCount = CalculateChemProtRoughSprayerCount(
        totalWorkingSolutionLiters,
        areaToBeSprayedAcres,
        sprayerVolumePerAcreLiters
      );
      const workingSolutionPerSprayerLiters = CalculateChemProtWorkingSolutionPerSprayerML(
        chemicalPerAcreML,
        workingSolutionPerAcreLiters,
        areaToBeSprayedAcres,
        sprayerVolumePerAcreLiters
      ...
      if (
        isNaN(totalWorkingSolutionLiters) ||
        totalWorkingSolutionLiters < 0 ||
        !isFinite(totalWorkingSolutionLiters)
      ) {
        isMathWorking = false;
      }
      ...
      if (
        isNaN(workingSolutionPerSprayerLiters) ||
        workingSolutionPerSprayerLiters < 0 ||
        !isFinite(workingSolutionPerSprayerLiters)
      ) {
        isMathWorking = false;
      }
  ...
  useEffect(() => {
    const subscription = form.watch((_) => {
      ...
      if (currentChemicalId) {
        const selectedChemicalData = plantsChems.find(
          (c) => c.chemical.id === currentChemicalId
        )?.chemical;
        if (selectedChemicalData) {
          ...
          if (
            convertedDosage !== chemicalPerAcreML &&
            form.getFieldState('chemicalPerAcreML').isDirty
          ) {
