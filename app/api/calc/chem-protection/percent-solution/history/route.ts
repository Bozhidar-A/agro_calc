...
import {
  GetChemProtPercentHistory,
  InsertChemProtPercentHistoryEntry,
...
export async function POST(req: NextRequest) {
  try {
    ...
    Log(
      ['calc', 'chem-protection', 'percent-solution', 'history'],
      `POST returned: ${JSON.stringify(chemProtPercentHistoryEntry)}`
    );
    return NextResponse.json({ success: true });
  } catch (error: unknown) {
    const errorMessage = (error as Error)?.message ?? 'An unknown error occurred';
    Log(
      ['calc', 'chem-protection', 'percent-solution', 'history', 'POST'],
...
export async function GET() {
  try {
    ...
    const errorMessage = (error as Error)?.message ?? 'An unknown error occurred';
    Log(
      ['calc', 'chem-protection', 'percent-solution', 'history', 'GET'],
