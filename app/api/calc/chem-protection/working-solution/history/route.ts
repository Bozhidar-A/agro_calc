...
import {
  ...
  InsertChemProtWorkingSolutionHistoryEntry,
...
export async function POST(req: NextRequest) {
  try {
    ...
    Log(
      ...
      `POST called with: ${JSON.stringify(entry)}`
    );
    const chemProtWorkingSolutionHistoryEntry =
      await InsertChemProtWorkingSolutionHistoryEntry(entry);
    Log(
      ['calc', 'chem-protection', 'working-solution', 'history'],
      `POST returned: ${JSON.stringify(chemProtWorkingSolutionHistoryEntry)}`
    );
    return NextResponse.json({ success: true });
  } catch (error: unknown) {
    const errorMessage = (error as Error)?.message ?? 'An unknown error occurred';
    Log(
      ['calc', 'chem-protection', 'working-solution', 'history', 'POST'],
...
export async function GET() {
  try {
    ...
    Log(
      ['calc', 'chem-protection', 'working-solution', 'history'],
      `GET returned: ${JSON.stringify(history)}`
    );
    return NextResponse.json({ success: true, data: history });
  } catch (error: unknown) {
    const errorMessage = (error as Error)?.message ?? 'An unknown error occurred';
    Log(
      ['calc', 'chem-protection', 'working-solution', 'history', 'GET'],
