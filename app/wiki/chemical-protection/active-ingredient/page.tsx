...
export default function WikiChemicalProtectionActiveIngredientListPage() {
  const [activeIngredients, setActiveIngredients] = useState<WikiActiveIngredient[]>([]);
  ...
  useEffect(() => {
    APICaller(
      ['wiki', 'chem protection', 'active ingredient', 'GET'],
      '/api/wiki/chemical-protection/all-active-ingredients',
      'GET'
    )
      .then((res) => {
        if (res.success) {
      ...
      .catch((error) => {
        Log(['wiki', 'chem protection', 'active ingredient', 'GET'], `POST failed with: ${error}`);
        setErrored(true);
        toast.error(translator(SELECTABLE_STRINGS.TOAST_ERROR_LOADING_DATA), {
          description: translator(SELECTABLE_STRINGS.TOAST_TRY_AGAIN_LATER),
        });
  ...
  const filteredActiveIngredients = activeIngredients.filter((entry) =>
        ...
        <CardContent className="pt-3 sm:pt-4 md:pt-6">
          <div className="flex flex-col items-center">
            <div className="w-full max-w-2xl space-y-3 sm:space-y-4">
              ...
              />
              {filteredActiveIngredients.map((entry) => (
                <Link
                  href={`/wiki/chemical-protection/active-ingredient/${entry.id}`}
                  key={entry.id}
