...
export default function WikiSowingPlantPage() {
  ...
  useEffect(() => {
    APICaller(['wiki', 'sowing', 'plant', 'POST'], `/api/wiki/sowing/plant`, 'POST', {
      id: params.plantId,
    })
      .then((res) => {
        if (res.success) {
              ...
              <div className="space-y-4">
                <div className="bg-green-50 dark:bg-black p-4 rounded-lg text-center">
                  <span className="font-semibold text-black dark:text-white">
                    {translator(SELECTABLE_STRINGS.SOWING_RATE_INPUT_COEFFICIENT_SECURITY)}:
                  </span>{' '}
                  <span className="text-black dark:text-white">
                    {plantData?.coefficientSecurity &&
                      renderParameterValue(plantData.coefficientSecurity)}
                  </span>
                </div>
                <div className="bg-green-50 dark:bg-black p-4 rounded-lg text-center">
                  <span className="font-semibold text-black dark:text-white">
                    {translator(SELECTABLE_STRINGS.SOWING_RATE_INPUT_WANTED_PLANTS_PER_M2)}:
                  </span>{' '}
                  <span className="text-black dark:text-white">
                    {plantData?.wantedPlantsPerMeterSquared &&
                      renderParameterValue(plantData.wantedPlantsPerMeterSquared)}
                  </span>
                </div>
                <div className="bg-green-50 dark:bg-black p-4 rounded-lg text-center">
                  <span className="font-semibold text-black dark:text-white">
                    {translator(SELECTABLE_STRINGS.SOWING_RATE_INPUT_MASS_PER_1000g_SEEDS)}:
                  </span>{' '}
                  <span className="text-black dark:text-white">
                ...
                <div className="bg-green-50 dark:bg-black p-4 rounded-lg text-center">
                  <span className="font-semibold text-black dark:text-white">
                    {translator(SELECTABLE_STRINGS.SOWING_RATE_INPUT_PURITY)}:
                  </span>{' '}
                  <span className="text-black dark:text-white">
                ...
                <div className="bg-green-50 dark:bg-black p-4 rounded-lg text-center">
                  <span className="font-semibold text-black dark:text-white">
                    {translator(SELECTABLE_STRINGS.SOWING_RATE_INPUT_GERMINATION)}:
                  </span>{' '}
                  <span className="text-black dark:text-white">
                ...
                <div className="bg-green-50 dark:bg-black p-4 rounded-lg text-center">
                  <span className="font-semibold text-black dark:text-white">
                    {translator(SELECTABLE_STRINGS.SOWING_RATE_INPUT_ROW_SPACING)}:
                  </span>{' '}
                  <span className="text-black dark:text-white">
