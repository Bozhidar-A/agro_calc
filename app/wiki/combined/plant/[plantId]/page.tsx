...
export default function WikiCombinedPlantPage() {
  ...
  useEffect(() => {
    APICaller(['wiki', 'combined', 'plant', 'POST'], `/api/wiki/combined/plant`, 'POST', {
      id: params.plantId,
    })
      .then((res) => {
        if (res.success) {
              ...
              <div className="space-y-4">
                <div className="bg-green-50 dark:bg-black p-4 rounded-lg text-center">
                  <span className="font-semibold text-black dark:text-white">
                    {translator(SELECTABLE_STRINGS.WIKI_COMBINED_PLANT_TYPE)}:
                  </span>{' '}
                  <span className="text-black dark:text-white">{plantData?.plantType}</span>
                </div>
                <div className="bg-green-50 dark:bg-black p-4 rounded-lg text-center">
                  <span className="font-semibold text-black dark:text-white">
                    {translator(SELECTABLE_STRINGS.WIKI_COMBINED_MIN_SEEDING_RATE)}:
                  </span>{' '}
                  <span className="text-black dark:text-white">
                    {convertSeedingRate(plantData?.minSeedingRate || 0)}{' '}
                ...
                <div className="bg-green-50 dark:bg-black p-4 rounded-lg text-center">
                  <span className="font-semibold text-black dark:text-white">
                    {translator(SELECTABLE_STRINGS.WIKI_COMBINED_MAX_SEEDING_RATE)}:
                  </span>{' '}
                  <span className="text-black dark:text-white">
                    {convertSeedingRate(plantData?.maxSeedingRate || 0)}{' '}
                    {unitOfMeasurement === UNIT_OF_MEASUREMENT_LENGTH.ACRES
                      ? translator(SELECTABLE_STRINGS.KG_ACRE)
                      : translator(SELECTABLE_STRINGS.KG_HECTARE)}
                  </span>
                ...
                <div className="bg-green-50 dark:bg-black p-4 rounded-lg text-center">
                  <span className="font-semibold text-black dark:text-white">
                    {translator(SELECTABLE_STRINGS.WIKI_COMBINED_PRICE_FOR_1KG_SEEDS)}:
                  </span>{' '}
                  <span className="text-black dark:text-white">
                    {plantData?.priceFor1kgSeedsBGN} {translator(SELECTABLE_STRINGS.BGN)}
                  </span>
                </div>
              </div>
            </div>
