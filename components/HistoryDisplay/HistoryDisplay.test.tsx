...
const chemProtPercentTabText = mockTranslateFunction(
  SELECTABLE_STRINGS.CHEMICAL_PROTECTION_PERCENT_SOLUTION_CALC_TITLE
);
const chemProtWorkingTabText = mockTranslateFunction(
...
describe('HistoryDisplay', () => {
  ...
  it('should show loading state initially', () => {
    ...
    renderWithRedux(() => <HistoryDisplay />, { preloadedState });
  ...
  it('should show "no history" message when all histories are empty', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(mockTranslateFunction(SELECTABLE_STRINGS.NO_HISTORY))
      ).toBeInTheDocument();
  ...
  it('should render all history tabs when data is available', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(
        screen.getByText(
          mockTranslateFunction(
            mockSowingRateHistory[0].plant.latinName as keyof typeof SELECTABLE_STRINGS
          )
        )
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByText(`${mockChemProtPercentHistory[0].desiredPercentage.toFixed(2)}%`)
      ).toBeInTheDocument();

      await user.click(chemProtWorkingTab);
      expect(
        screen.getByText(
          `${mockChemProtWorkingSolutionHistory[0].totalChemicalForAreaLiters.toFixed(2)} L`
        )
      ).toBeInTheDocument();
    });
  });
  ...
  it('should filter sowing rate history by search query', async () => {
    ...
    const searchInput = screen.getByPlaceholderText(
      mockTranslateFunction(SELECTABLE_STRINGS.SEARCH_PLACEHOLDER)
    );
    await user.type(
      searchInput,
      mockTranslateFunction(
    ...
    expect(
      screen.queryByText(
        mockTranslateFunction(
          mockCombinedDataHistory[0].plants[1].plant.latinName as keyof typeof SELECTABLE_STRINGS
        )
      )
  ...
  it('should display warning banner for invalid data', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(mockTranslateFunction(SELECTABLE_STRINGS.WARNING_OUTSIDE_SUGGESTED_PARAMS))
      ).toBeInTheDocument();
    });
  });
  ...
  it('should display all data fields for combined data history', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(
        screen.getByText(
          mockTranslateFunction(
            mockCombinedDataHistory[0].plants[0].plant.latinName as keyof typeof SELECTABLE_STRINGS
          )
        )
      ).toBeInTheDocument();
      expect(
        screen.getByText(`${mockCombinedDataHistory[0].plants[0].seedingRate.toFixed(2)}`)
      ).toBeInTheDocument();
      expect(
        screen.getByText(`${mockCombinedDataHistory[0].plants[0].participation.toFixed(2)}%`)
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          `${mockCombinedDataHistory[0].plants[0].pricePerAcreBGN.toFixed(2)} ${mockTranslateFunction(SELECTABLE_STRINGS.BGN)}`
        )
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          `${mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_FINAL_PRICE)}: ${mockCombinedDataHistory[0].totalPrice.toFixed(2)} ${mockTranslateFunction(SELECTABLE_STRINGS.BGN)}`
        )
      ).toBeInTheDocument();
    });
  });
  ...
  it('should display all data fields for chemical protection percent solution history', async () => {
    ...
    await waitFor(async () => {
      ...
      expect(
        screen.getByText(`${mockChemProtPercentHistory[0].desiredPercentage.toFixed(2)}%`)
      ).toBeInTheDocument(); // desiredPercentage
      expect(
        screen.getByText(`${mockChemProtPercentHistory[0].sprayerVolume.toFixed(2)} L`)
      ).toBeInTheDocument(); // sprayerVolume
      expect(
        screen.getByText(`${mockChemProtPercentHistory[0].calculatedAmount.toFixed(2)} ml/g`)
      ).toBeInTheDocument(); // calculatedAmount
    });
  });
  ...
  it('should display all data fields for chemical protection working solution history', async () => {
    ...
    await waitFor(async () => {
      ...
      const plantName = mockTranslateFunction(
        mockChemProtWorkingSolutionHistory[0].plant.latinName as keyof typeof SELECTABLE_STRINGS
      );
      const chemicalName = mockTranslateFunction(
        mockChemProtWorkingSolutionHistory[0].chemical.nameKey as keyof typeof SELECTABLE_STRINGS
      );
      ...
      ).toBeInTheDocument(); // totalChemicalForAreaLiters
      expect(
        screen.getByText(
          `${mockChemProtWorkingSolutionHistory[0].totalWorkingSolutionForAreaLiters.toFixed(2)} L`
        )
      ).toBeInTheDocument(); // totalWorkingSolutionForAreaLiters
      expect(
        screen.getByText(`${mockChemProtWorkingSolutionHistory[0].roughSprayerCount.toFixed(2)}`)
      ).toBeInTheDocument(); // roughSprayerCount
      expect(
        screen.getByText(
          `${mockChemProtWorkingSolutionHistory[0].chemicalPerSprayerML.toFixed(2)} ml`
        )
      ).toBeInTheDocument(); // chemicalPerSprayerML
    });
  });
