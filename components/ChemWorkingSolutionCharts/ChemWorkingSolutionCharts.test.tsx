...
describe('ChemWorkingSolutionCharts', () => {
  ...
  describe('Rendering', () => {
    it('renders all chart sections when data is provided', () => {
      renderWithRedux(() => <ChemWorkingSolutionCharts data={mockData} />, { preloadedState });

      // Check for main title and description
      expect(
        screen.getByText(mockTranslateFunction(SELECTABLE_STRINGS.CHEM_PROT_WORKING_SOLUTION_VIZ))
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          mockTranslateFunction(SELECTABLE_STRINGS.CHEM_PROT_WORKING_SOLUTION_VIZ_GENERAL)
        )
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByText(
          mockTranslateFunction(SELECTABLE_STRINGS.CHEM_PROT_WORKING_SOLUTION_VIZ_ALL_ELEMENTS)
        )
      ).toBeInTheDocument();
      expect(
        screen.getByText(
      ...
      expect(
        screen.getByText(
          mockTranslateFunction(SELECTABLE_STRINGS.CHEM_PROT_WORKING_SOLUTION_VIZ_PARTICIPATION)
        )
      ).toBeInTheDocument();
    ...
    it('renders nothing when no data is provided', () => {
      renderWithRedux(() => <ChemWorkingSolutionCharts data={null as any} />, { preloadedState });

      // Check that main title is not rendered
      expect(
        screen.queryByText(mockTranslateFunction(SELECTABLE_STRINGS.CHEM_PROT_WORKING_SOLUTION_VIZ))
  ...
  describe('Chart Data Display', () => {
    ...
    it('displays correct efficiency metrics', () => {
      ...
      expect(
        screen.getByText(
          mockTranslateFunction(SELECTABLE_STRINGS.CHEM_PROT_WORKING_SOLUTION_TOTAL_CHEMICAL)
        )
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          mockTranslateFunction(SELECTABLE_STRINGS.CHEM_PROT_WORKING_SOLUTION_TOTAL_SOLUTION)
        )
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          mockTranslateFunction(SELECTABLE_STRINGS.CHEM_PROT_WORKING_SOLUTION_SPRAYER_COUNT)
        )
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          mockTranslateFunction(SELECTABLE_STRINGS.CHEM_PROT_WORKING_SOLUTION_CHEMICAL_PER_SPRAYER)
        )
      ).toBeInTheDocument();

      // Check for participation label in chart
      const barElements = screen.getAllByTestId('bar');
      expect(barElements[barElements.length - 1]).toHaveAttribute(
        'data-name',
        mockTranslateFunction(
          SELECTABLE_STRINGS.CHEM_PROT_WORKING_SOLUTION_VIZ_PARTICIPATION_PERCENTAGE
        )
