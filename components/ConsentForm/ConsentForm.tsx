...
export default function ConsentForm({ open, onOpenChange }: ConsentDialogProps) {
  ...
  const { handleSubmit, setValue, watch, reset } = useForm<z.infer<typeof ConsentSchema>>({
      ...
      <DialogContent className="sm:max-w-md space-y-6">
        <DialogHeader>
          <DialogTitle className="text-center">
            {translate(SELECTABLE_STRINGS.GDPR_CONSENT_TITLE)}
          </DialogTitle>
        ...
        >
          ...
          <div className="space-y-2 rounded-lg border p-4 text-left">
            ...
            <p className="text-sm text-muted-foreground">
              {translate(SELECTABLE_STRINGS.GDPR_CONSENT_PREFERENCES_DESCRIPTION)}
            </p>
          </div>
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            <Button type="button" variant="outline" onClick={OnAcceptAll} className="font-semibold">
