...
import { Percent, TreesIcon as Plant } from 'lucide-react';
...
export function SeedCombinedSection({
  name,
  title,
  maxPercentage,
  form,
  dbData,
}: SeedCombinedSectionProps) {
  ...
  return (
    <Card
      ...
      id={name === 'legume' ? 'perennialLegumeSection' : 'perennialCerealSection'}
    >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0 mt-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-base sm:text-lg">
              {translator(SELECTABLE_STRINGS.COMBINED_TOTAL_PARTICIPATION_LABEL)}:{' '}
              {participation.toFixed(1)}%
            </span>
          </div>
          <span
            className={`text-base sm:text-lg font-medium ${isOverLimit ? 'text-red-500' : 'text-green-500'}`}
            id={name === 'legume' ? 'perennialLegumeParticipation' : 'perennialCerealParticipation'}
          >
      ...
      <CardContent className="p-3 sm:p-4">
        ...
        <div className="grid gap-3 sm:gap-4">
          <div className="hidden md:grid grid-cols-6 gap-4 font-medium text-sm md:text-base border-b pb-2">
            <div className="text-center" id="activeCheckbox">
              {translator(SELECTABLE_STRINGS.COMBINED_ACTIVE)}
            </div>
            ...
            <div id="participationPercent">
              {translator(SELECTABLE_STRINGS.COMBINED_PARTICIPATION_PERCENT)}
            </div>
            <div id="sowingRateMixture">
              {translator(SELECTABLE_STRINGS.COMBINED_SOWING_RATE_MIX)}
            </div>
            <div id="seedPrice">
