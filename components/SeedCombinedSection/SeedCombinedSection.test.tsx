...
describe('SeedCombinedSection', () => {
    ...
    it('renders section with correct title and icons', () => {
        ...
        expect(screen.getByText(`${mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_TOTAL_PARTICIPATION_LABEL)}: 0.0%`)).toBeInTheDocument();
        expect(screen.getByText(`${mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_MAX_PARTICIPATION)}: 60%`)).toHaveClass('text-green-500');
    });
    ...
    it('shows warning when participation exceeds max percentage', () => {
        ...
        expect(screen.getByText(`${mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_MAX_PARTICIPATION)}: 60%`)).toHaveClass('text-red-500');
        expect(screen.getByText(mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_VALUES_OUTSIDE_LIMIT))).toBeInTheDocument();
    });
    ...
    it('renders mobile view headers correctly', () => {
        ...
        const mobileHeaders = screen.getAllByText(mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_PLANT));
        // The mobile header is the one inside the md:hidden container
        const mobileHeader = mobileHeaders.find(header =>
            header.closest('.md\\:hidden')
        );
