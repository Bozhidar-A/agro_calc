...
describe('MeasurementSwitcher', () => {
  ...
  it('renders with correct initial measurement unit', () => {
    renderWithRedux(() => <MeasurementSwitcher />, { preloadedState });
  ...
  it('updates measurement unit when a new option is selected', async () => {
    const user = userEvent.setup();
    const { store } = renderWithRedux(() => <MeasurementSwitcher />, { preloadedState });
  ...
  it('handles switching between measurement units', async () => {
    ...
    expect(store.getState().local.unitOfMeasurementLength).toBe(
      UNIT_OF_MEASUREMENT_LENGTH.HECTARES
    );
  ...
  it('renders with acres as default when no unit is specified', () => {
    ...
    renderWithRedux(() => <MeasurementSwitcher />, { preloadedState: stateWithoutUnit });
