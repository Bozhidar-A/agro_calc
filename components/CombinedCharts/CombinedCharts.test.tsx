...
describe('CombinedCharts', () => {
    ...
    describe('Rendering', () => {
        ...
        it('renders all chart sections when data is provided', () => {
            renderWithRedux(
                () => <CombinedCharts data={mockData} />,
                { preloadedState }
            );

            // Check for main title and description
            expect(screen.getByText(mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_VISUALIZATION_TITLE))).toBeInTheDocument();
            expect(screen.getByText(mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_VISUALIZATION_DESCRIPTION))).toBeInTheDocument();

            // Check for chart titles
            expect(screen.getByText(mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_SOWING_RATE_BY_PLANT_TYPE))).toBeInTheDocument();
            expect(screen.getByText(mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_PARTICIPATION_DISTRIBUTION))).toBeInTheDocument();
            expect(screen.getByText(mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_PRICE_PER_ACRE_COMPARISON))).toBeInTheDocument();
            expect(screen.getByText(mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_SOWING_RATE_COMPARISON))).toBeInTheDocument();
        ...
        it('renders nothing when no data is provided', () => {
            renderWithRedux(
                () => <CombinedCharts data={null} />,
                { preloadedState }
            );

            // Check that main title is not rendered
            expect(screen.queryByText(mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_VISUALIZATION_TITLE))).not.toBeInTheDocument();
        });
    ...
    describe('Unit Switching', () => {
        it('displays hectare units when hectares is selected', () => {
            ...
            renderWithRedux(
                () => <CombinedCharts data={mockData} />,
                { preloadedState: hectaresState }
            );

            // Check that hectares title is displayed
            expect(screen.getByText(mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_PRICE_PER_HECTARE_COMPARISON))).toBeInTheDocument();
        });
    ...
    describe('Chart Data Display', () => {
        ...
        it('displays correct plant names in charts', () => {
            ...
            mockData.plants.forEach(plant => {
                const translatedName = mockTranslateFunction(plant.plantLatinName as keyof typeof SELECTABLE_STRINGS);
        ...
        it('displays correct chart legends', () => {
            ...
            expect(barNames).toContain(mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_SOWING_RATE_SINGLE_PLANT));
            expect(barNames).toContain(mockTranslateFunction(SELECTABLE_STRINGS.COMBINED_SOWING_RATE_MIX_PLANT));
        });
    });
}); 