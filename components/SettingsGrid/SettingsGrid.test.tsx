...
describe('SettingsGrid', () => {
  ...
  it('renders all setting labels with correct translations', () => {
    ...
    //check theme label
    const themeLabel = screen.getByText(
      `${mockTranslateFunction(SELECTABLE_STRINGS.SETTINGS_THEME)}:`
    );
    expect(themeLabel).toBeInTheDocument();
    expect(themeLabel).toHaveClass('font-medium', 'text-sm', 'sm:text-base');

    //check language label
    const langLabel = screen.getByText(
      `${mockTranslateFunction(SELECTABLE_STRINGS.SETTINGS_LANGUAGE)}:`
    );
    expect(langLabel).toBeInTheDocument();
    expect(langLabel).toHaveClass('font-medium', 'text-sm', 'sm:text-base');

    //check measurement label
    const measureLabel = screen.getByText(
      `${mockTranslateFunction(SELECTABLE_STRINGS.SETTINGS_PREF_UNIT_OF_MEASUREMENT)}:`
    );
  ...
  it('renders with correct layout classes', () => {
    ...
    expect(settingsContainer).toHaveClass(
      'space-y-6',
      'p-4',
      'sm:p-6',
      'bg-card',
