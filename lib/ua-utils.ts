...
export async function FormatUserAccessInfo(
  loc: UserGPSLoc | null,
  ua: UserAgentNext | null,
  ...
  if (loc) {
    ...
    locationStr = location
      ? `${location.city || location.town || location.village || '?'}, ${location.country || '?'}`
      : '?';
  }
...
export async function FromRequestFormatUserAccessInfo(
  request: NextRequest,
  userAgent: UserAgentNext | null,
  OAuthProvider: string | null
): Promise<string> {
