import { FormatUserAgent, FormatUserAccessInfo, ReverseGeocode, FromRequestFormatUserAccessInfo, ReadUserLocationFromQueryParams, CreateTempUserLocCookies, ReadTempUserLocationCookies, DeleteTempUserLocationCookies } from "../ua-utils";
...
describe("ua-utils", () => {
    ...
    test("FormatUserAccessInfo and FromRequestFormatUserAccessInfo use location/UA/OAuth correctly", async () => {
        ...
        (fakeReq.headers as any).get = (k: string) => (k === "x-user-lat" ? "1" : k === "x-user-lon" ? "2" : null);
        ...
        const out = await FromRequestFormatUserAccessInfo(fakeReq as NextRequest, userAgent as any, "git");
    ...
    test("ReadUserLocationFromQueryParams and temp cookie helpers", () => {
        ...
        const resp: Partial<NextResponse> = { cookies: { set: (k: string, v: string) => map.set(k, v), get: (k: string) => ({ value: map.get(k) }), delete: (k: string) => map.delete(k) } as any } as any;
        ...
        const req: Partial<NextRequest> = { cookies: { get: (k: string) => ({ value: k === "temp_user_lat" ? "10" : k === "temp_user_lon" ? "20" : undefined }) } } as any;
