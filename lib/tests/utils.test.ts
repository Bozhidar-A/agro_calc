...
describe('GetParameterData', () => {
    ...
    it('should return correct data for ABOVE_ZERO', () => {
        ...
        expect(GetParameterData(param)).toEqual({ type: 'aboveZero', unit: 'm', minSliderVal: 0, maxSliderVal: 10, step: 2 });
    });
    it('should return correct data for SLIDER', () => {
        const param = { type: CalculatorValueTypes.SLIDER, unit: 'l', minSliderVal: 1, maxSliderVal: 5, step: 0.5 };
        expect(GetParameterData(param)).toEqual({ type: 'slider', unit: 'l', step: 0.5, minSliderVal: 1, maxSliderVal: 5 });
    });
...
describe('GetDisplayValue', () => {
    it('should convert to hectares if unit is HECTARES', () => {
        expect(GetDisplayValue(2, UNIT_OF_MEASUREMENT_LENGTH.HECTARES)).toBe(20.00);
    });
    it('should keep value if unit is ACRES', () => {
        expect(GetDisplayValue(2, UNIT_OF_MEASUREMENT_LENGTH.ACRES)).toBe(2.00);
    });
});
