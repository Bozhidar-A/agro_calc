...
describe('DecodeTokenContent', () => {
  ...
  it('returns success true and logs error if exception is thrown', async () => {
    ...
    expect(Log).toHaveBeenCalledWith(
      ['auth', 'logout', 'frontend'],
      expect.stringContaining('DecodeTokenContent failed with: fail!')
    );
  });
  ...
  it("decodes only access token when target='access'", async () => {
    ...
    (BackendVerifyToken as jest.Mock).mockResolvedValueOnce([
      true,
      { payload: { userId: 'only-access' } },
    ]); // access only
