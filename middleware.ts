...
export async function middleware(request: NextRequest) {
  const routeDefinitions = {
    protected: {
      ...
      afterAuthPages: ['/auth/login', '/auth/register'],
    },
  };
  ...
  const isProtectedRoute = ArrayContainsAndItemsStartsWith(
    [...routeDefinitions.protected.api, ...routeDefinitions.protected.pages],
    pathname
  );

  const isAfterAuthRoute = ArrayContainsAndItemsStartsWith(
    [...routeDefinitions.protected.afterAuthAPI, ...routeDefinitions.protected.afterAuthPages],
    pathname
  );
    ...
    if (!validAccessToken) {
      ...
      try {
        ...
        if (!logoutAction.success) {
          Log(
            ['middleware', 'logout', 'error'],
            `BackendLogout failed with: ${logoutAction.message}`
          );
        }

        return NextResponse.redirect(
          new URL('/auth/login?updateAuthState=refreshTokenExpired', request.url)
        );
      }
    }
  }
